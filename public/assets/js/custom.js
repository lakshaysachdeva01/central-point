...
$(function () {
    ...
    // Offers 2 
    $('.offers2 .item').on('mouseenter', function () {
    ...
    // Handle clicks - allow navigation if it's a link with href
    $('.offers2 .item').on('click', function (e) {
        // If it has an href attribute, allow navigation (don't prevent default)
        if ($(this).attr('href') && $(this).attr('href') !== '#') {
            // Allow navigation to happen
            return true;
        }
        
        // Otherwise, handle as before (prevent default for non-link items)
        var tab_id = $(this).attr('data-tab');
        $('.offers2 .item').removeClass('current');
        $(this).addClass('current');
